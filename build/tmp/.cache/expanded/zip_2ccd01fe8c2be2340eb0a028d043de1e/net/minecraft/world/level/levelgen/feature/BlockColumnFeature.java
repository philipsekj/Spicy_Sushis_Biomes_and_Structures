...
public class BlockColumnFeature extends Feature<BlockColumnConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<BlockColumnConfiguration> p_190791_) {
      ...
      BlockColumnConfiguration blockcolumnconfiguration = (BlockColumnConfiguration)p_190791_.config();
      ...
      if (j == 0) {
         ...
         for(int k1 = 0; k1 < i; ++k1) {
            int i1 = aint[k1];
            if (i1 != 0) {
               BlockColumnConfiguration.Layer layer = (BlockColumnConfiguration.Layer)blockcolumnconfiguration.layers().get(k1);

               for(int j1 = 0; j1 < i1; ++j1) {
                  worldgenlevel.setBlock(blockpos$mutableblockpos1, layer.state().getState(randomsource, blockpos$mutableblockpos1), 2);
